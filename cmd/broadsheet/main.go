// Command broadsheet is the debug / ops CLI.
package main
...
func main() {
	rootCmd := &cobra.Command{
		...
		Long: `broadsheet fetches newspaper front pages from freedomforum.org, rasterizes
them, and serves them on rotation.

This CLI is for ops/debug. The server lives in cmd/broadsheet-server.`,
	}
...
func mustEngine() *broadsheet.Engine {
	var ce cliEnv
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "init broadsheet:", err)
		os.Exit(1)
	}
	return p
}
...
func newFetchCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(_ *cobra.Command, args []string) {
			...
			opts := broadsheet.RenderOptions{OutputWidth: outWidth}
